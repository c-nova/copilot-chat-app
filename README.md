...
- **Multi-session orchestration**: open an existing session as an Orchestrator, or start a new chat with
  **Orchestrator mode** enabled. Talk only to the Conductor while it creates or reuses read-only Player sessions,
  including Players on configured peer servers. Main and child views follow new activity automatically until you
  scroll away to inspect older output.
...
- **Selectable message text**: tap **🔎 テキスト選択** under a message to open its Markdown source in a
  read-only selectable view, with a Copy All command. This works in regular chats and both Conductor/Player
  panes, and follows the configured chat font size.
...
前提条件: Node.js 20.19以上(または22.12以上)、サーバーを動かすマシンに GitHub Copilot CLI が
インストール・ログイン済み(`copilot login`)であること。
...
macOSでは、`scripts/build-mac.sh`がサーバービルド成功後にインストール済みLaunchAgentを自動再起動し、
新しい`server/dist`を即座に読み込みます。`.env`だけを変更した場合は`scripts/restart-server-mac.sh`を
使ってください。Windowsで再ビルド後のコードや設定変更を反映する場合は
`scripts/restart-server-windows.ps1`を使います。
...
```bash
# iOS実機(既定はRelease。事前に署名/プロビジョニング設定が必要)
./scripts/deploy-ipad.sh --run                 # USB接続中のiPadを自動検出
./scripts/deploy-ipad.sh --iphone --run        # USB接続中のiPhoneを自動検出
./scripts/deploy-ipad.sh --clean --debug --run # クリーンDebugビルド後にインストール・起動
```
...
- **マルチセッション・オーケストレーション**: 既存セッションをOrchestratorで開くか、新規チャットで
  **Orchestratorモード**を有効にして開始できます。人間はConductorとのみ会話し、Conductorが読み取り専用の
  Playerセッションを作成・再利用します。設定済みピアサーバー上のPlayerも利用できます。メイン/子画面は、
  過去ログを見るために手動で上へスクロールするまでは最新の処理へ自動追従します。
...
- **メッセージ本文の選択**: メッセージ下の**🔎 テキスト選択**をタップすると、Markdownソースを読み取り専用の
  選択可能画面で開き、部分選択またはCopy Allできます。通常チャットとConductor/Playerの両ペインで使え、
  Settingsのチャットフォントサイズにも連動します。
