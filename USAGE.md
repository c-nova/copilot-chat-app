...
An already-running Node process must be restarted to load rebuilt `server/dist` or a changed `server/.env`.
On macOS, `scripts/build-mac.sh` automatically restarts an installed LaunchAgent after a successful
server build. Use `scripts/restart-server-mac.sh` after an `.env`-only edit. On Windows, run
`scripts/restart-server-windows.ps1` after rebuilding or changing configuration.
...
notification, it also mirrors to a paired Apple Watch by default - verified on real hardware, though
only while the iPhone is locked/asleep; iOS doesn't mirror to the Watch while you're actively using
the phone). This works entirely on the server side - the client app doesn't need to be running (in
the foreground or background) for notifications to arrive, only the server does.
...
4. **Build, then run**, passing your signing identity and the plain UDID (no `:v2:...` prefix — that format
   ...
   After the one-time signing/profile setup, the repository script wraps the build/install commands above.
   It defaults to a Release build and auto-detects a USB-connected device:
   ```bash
   cd copilot-chat-app
   ./scripts/deploy-ipad.sh --run                 # iPad, Release
   ./scripts/deploy-ipad.sh --iphone --run        # iPhone, Release
   ./scripts/deploy-ipad.sh --clean --debug --run # clean Debug build
   ```
   Use `IOS_DEVICE_UDID=...` or `IOS_CODESIGN_KEY="Apple Development: ..."` before the command to override
   auto-detection without editing the script. Run `./scripts/deploy-ipad.sh --help` for all options.
...
For a signed physical iPhone/iPad build, use [`scripts/deploy-ipad.sh`](scripts/deploy-ipad.sh) as shown in
the iOS section above.
...
The model picker above the composer controls subsequent turns on that chat page. It loads the models currently
available to the signed-in Copilot account through the official SDK. Leave **Server default** selected to use
`COPILOT_MODEL` from the server environment, or the CLI default when that is unset. The selection is not saved.
...
- **Select or copy message text**: tap **🔎 テキスト選択** below any message to open its Markdown source in a
   read-only selectable view. Select a portion normally, or use **Copy All**. The view follows the chat font size.
...
Use Orchestrator mode when one main session should coordinate several worker sessions:

- Start a new chat with **Orchestrator mode** enabled, or open an existing session's **⋯** menu and choose
   **🎼 Orchestratorで開く**.
- You type only in the upper Conductor pane. Player panes below are read-only; the Conductor can create,
   attach, or continue them through its session-control tools. The **＋ 子セッション** button also lets you
   create or attach one manually.
- With multiple app server profiles, manually created Players can target another server. For AI-selected
   cross-machine Players, configure matching `PEER_SERVERS` entries in the server `.env` files.
- Conductor and Player panes follow new activity automatically. If you scroll upward to inspect older output,
   auto-follow pauses so incoming updates do not pull the view away.
- Sessions are marked **🎼 Conductor** and **🎻 Player** on Home. If a former Conductor no longer has any
   Players, tapping it opens as a regular chat again.

Player messages render as Markdown, and **🔎 テキスト選択** is available in both Conductor and Player panes.
...
実行中のNodeプロセスへ、再ビルドした`server/dist`や変更した`server/.env`を読み込ませるには再起動が必要です。
macOSでは`scripts/build-mac.sh`がサーバービルド成功後にインストール済みLaunchAgentを自動再起動します。
`.env`だけを変更した場合は`scripts/restart-server-mac.sh`、Windowsで再ビルドや設定変更を反映する場合は
`scripts/restart-server-windows.ps1`を実行してください。
...
ペアリング済みのApple Watchにも実機で確認済みです。ただしiPhoneがロック/スリープ中でないと
Watch側にはミラーリングされません — 操作中は重複防止のためiPhone側にしか表示されない仕様)。
この通知はサーバー側だけで完結しており、クライアントアプリはフォアグラウンド・バックグラウンド
どちらでも起動している必要はありません(サーバーさえ動いていればOK)。
...
4. **ビルドしてから実行**。署名IDとプレーンなUDID(`:v2:...`のような接頭辞を付けると起動がハングします)を
   ...
   初回の署名/プロファイル設定が済んだ後は、上のビルド・インストール手順をリポジトリのスクリプトで
   まとめて実行できます。既定はReleaseビルドで、USB接続中の端末を自動検出します:
   ```bash
   cd copilot-chat-app
   ./scripts/deploy-ipad.sh --run                 # iPad、Release
   ./scripts/deploy-ipad.sh --iphone --run        # iPhone、Release
   ./scripts/deploy-ipad.sh --clean --debug --run # クリーンDebugビルド
   ```
   自動検出を上書きする場合は、スクリプトを編集せずコマンドの前に`IOS_DEVICE_UDID=...`または
   `IOS_CODESIGN_KEY="Apple Development: ..."`を指定できます。全オプションは
   `./scripts/deploy-ipad.sh --help`で確認できます。
...
インストールする」を参照してください。署名済みのiPhone/iPad実機ビルドには、上のiOSセクションで説明した
[`scripts/deploy-ipad.sh`](scripts/deploy-ipad.sh)を使います。
...
入力欄の上にあるモデルPickerは、そのチャット画面で以降に送るturnへ適用されます。ログイン中のCopilot
アカウントで現在利用できるモデルを公式SDK経由で読み込みます。**Server default**のままならサーバー環境の
`COPILOT_MODEL`、それも未設定ならCLIのデフォルトを使います。選択内容は保存されません。
...
- **メッセージ本文を選択/コピー**: 各メッセージ下の**🔎 テキスト選択**をタップすると、Markdownソースを
   読み取り専用の選択可能画面で開きます。通常の部分選択または**Copy All**が使え、チャットフォントサイズにも
   連動します。
...
### 複数セッションをオーケストレーションする

1つのメインセッションから複数のワーカーを協調させたい場合はOrchestratorモードを使います:

- 新規チャットで**Orchestratorモード**を有効にするか、既存セッションの**⋯**メニューから
   **🎼 Orchestratorで開く**を選びます。
- 人間が入力するのは上部のConductorペインだけです。下部のPlayerペインは読み取り専用で、Conductorが
   session-controlツールを通して作成・アタッチ・継続します。**＋ 子セッション**ボタンから手動で作成・
   アタッチすることもできます。
- アプリに複数サーバープロファイルがあれば、手動作成するPlayerの配置先を別サーバーから選べます。
   AIがクロスマシンのPlayerを選ぶには、サーバーの`.env`に対応する`PEER_SERVERS`設定が必要です。
- Conductor/Playerペインは新しい処理へ自動追従します。古い出力を見るため上へスクロールすると、受信中の
   更新で勝手に最下部へ戻されないよう自動追従を一時停止します。
- Homeでは**🎼 Conductor** / **🎻 Player**バッジで識別できます。Playerが1つも残っていない元Conductorは、
   タップすると通常チャットとして開きます。

Playerの本文もMarkdownで表示され、Conductor/Playerのどちらでも**🔎 テキスト選択**を利用できます。
